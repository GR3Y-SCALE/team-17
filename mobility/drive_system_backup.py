...
class DriveSystem:
    ...
    Manages motor initialization, velocity control, and a PID feedback loop for precise speed regulation based on encoder feedback.
    """
    ...
    def __init__(
        ...
        self.turn_gain = turn_gain
                ...
                # in __init__
        self.odom_scale = 1.0   # >1 increases computed distance, <1 decreases
    ...
    def set_pid_gains(self, kp: Optional[float] = None, ki: Optional[float] = None, kd: Optional[float] = None):
        ...
        with self._lock:
            if kp is not None:
                self.kp_l = self.kp_r = kp
            if ki is not None:
                self.ki_l = self.ki_r = ki
            if kd is not None:
                self.kd_l = self.kd_r = kd
    ...
    def _pid_loop(self):
        ...
        try:
            while self._running:
                ...
                if abs(rpm_l_target) < 0.5:
                    self._integral_l = 0.0
                if abs(rpm_r_target) < 0.5:
                    self._integral_r = 0.0
    ...
    def _rpm_to_mps(self, rpm: float) -> float:
        mps = (rpm / 60.0) * (2.0 * math.pi * self.wheel_radius_m)
        return self.odom_scale * mps   # <-- APPLY THE SCALE
    ...
    def _reset_encoders(self):
        """
        Best-effort encoder reset. If board exposes a dedicated 'clear encoder' method, call it here instead.
        This version toggles enable and waits briefly.
        """
        try:
            self.board.set_encoder_enable(self.board.ALL)
        except Exception as e:
    ...
    def turn_degrees(
        ...
        # Integrate distance from measured RPMs
        dist_l = 0.0
        dist_r = 0.0
        last_t = time.time()
        try:
            while True:
                ...
                # Convert remaining arc back to angle (deg)
